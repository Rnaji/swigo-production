    ...
    if request.method == 'POST':
        try:
            ...
            # 5. CAS ZONE DESSERVIE - Livraison disponible
                ...
                temps_livraison = getattr(ville_desservie, 'temps_gisors', 20)
                dt_depart = datetime.combine(dt, heure_estimee)
                heure1 = (dt_depart + timedelta(minutes=temps_livraison)).time().strftime('%H:%M')
                heure2 = (dt_depart + timedelta(minutes=temps_livraison + 15)).time().strftime('%H:%M')
...
def get_cart_items(panier):
    ...
    for article in articles:
        ...
        # Initialiser les variables pour cet article
        options_list = []
        plat_nom = "Inconnu"
        categorie_nom = "Autres"
        accompagnement_info = None

        # 🆕 CROUSTY PERSONNALISÉ
        if article.plat and article.plat.type_plat == 'crousty' and article.options_crousty.exists():
            ...
            for option in article.options_crousty.all():
                ...
                option_text = f"+{option.nom}"
                if option.prix_supplement > 0:
                    option_text += f" (+{option.prix_supplement}€)"
                options_par_categorie[option.categorie].append(option_text)
            
            ordre_categories = ['fromage', 'sauce_rouge', 'sauce_erc', 'prot_veggie', 'supplement']
            for categorie in ordre_categories:
                if categorie in options_par_categorie:
                    for option_text in options_par_categorie[categorie]:
                        options_list.append(option_text)

        # 🆕 POULET PERSONNALISÉ
        elif article.plat and article.plat.type_plat == 'poulet' and (article.options_poulet.exists() or getattr(article, 'options_poulet_avec_quantites', [])):
            plat_nom = article.plat.nom
            ...
            if hasattr(article, 'options_poulet_avec_quantites') and article.options_poulet_avec_quantites:
                ...
                for option_id, quantite in options_counter.items():
                    try:
                        ...
                        options_avec_noms[option_id] = {
                            'nom': option.nom,
                            'quantite': quantite,
                            'categorie': option.categorie,
                            'prix_supplement': option.prix_supplement
                        }
                ...
                for option_id, option_data in options_avec_noms.items():
                    if option_data['categorie'] == 'assaisonnement':
                        option_text = f"🔴 +{option_data['nom']}"
                    elif option_data['categorie'] == 'accompagnement':
                        option_text = f"🟢 +{option_data['nom']}"
                    elif option_data['categorie'] == 'sauce':
                        option_text = f"🔵 +{option_data['nom']}"
                    else:
                        option_text = f"🟡 +{option_data['nom']}"
                    
                    if option_data['prix_supplement'] > 0:
                        option_text += f" (+{option_data['prix_supplement']}€)"
                    if option_data['quantite'] > 1:
                        option_text += f" × {option_data['quantite']}"
                    
                    if option_data['categorie'] == 'assaisonnement':
                        assaisonnements.append(option_text)
                    elif option_data['categorie'] == 'accompagnement':
                        accompagnements.append(option_text)
                    elif option_data['categorie'] == 'sauce':
                        sauces.append(option_text)
                    else:
                        supplements.append(option_text)

                options_list = assaisonnements + accompagnements + sauces + supplements
                
            else:
                print(f"   - Options trouvées: {article.options_poulet.count()}")
                ...
                for option in article.options_poulet.all():
                    ...
                    existing_option = next((item for item in options_par_categorie[option.categorie] 
                                          if item['id'] == option.id), None)
                    
                    if existing_option:
                        existing_option['quantite'] += 1
                    else:
                        option_text = f"+{option.nom}"
                        if option.prix_supplement > 0:
                            option_text += f" (+{option.prix_supplement}€)"
                ...
                ordre_categories = ['assaisonnement', 'accompagnement', 'sauce', 'supplement']
                
                for categorie in ordre_categories:
                    if categorie in options_par_categorie:
                        for option_data in options_par_categorie[categorie]:
                            if option_data['quantite'] > 1:
                                options_list.append(f"{option_data['nom']} × {option_data['quantite']}")
                            else:
                                options_list.append(option_data['nom'])
                
                print(f"   - Options formatées: {options_list}")
        ...
        # 🥘 COUSCOUS PERSONNALISÉ
            ...
            for choix in couscous.choixviandecouscous_set.all():
                (incluses if choix.est_incluse else supplements).append(
                    choix.viande.nom if choix.est_incluse else f"+{choix.viande.nom} (+{choix.viande.supplement_extra} €)"
                )

            if incluses:
                incluses_count = Counter(incluses)  # ✅ Plus d'erreur de portée
                for nom, count in incluses_count.items():
                    if count > 1:
                        options_list.append(f"{nom} × {count}")
                    else:
            ...
            accompagnements = couscous.accompagnements.all()
            if accompagnements:
                for accompagnement in accompagnements:
                    options_list.append(f"+{accompagnement.nom}")

            if supplements:
                for supplement in supplements:
                    options_list.append(supplement)
        ...
        # 🧂 SALADE PERSONNALISÉE
            ...
            if salade.sauce:
                sauce_txt = f"+{salade.sauce.nom}"
                supp = salade.sauce.prix_supplement_salade or Decimal('0.00')
                if supp > 0:
                    sauce_txt += f" (+{supp:.2f} €)"
                options_list.append(sauce_txt)
        ...
        # 🍱 MENU PERSONNALISÉ
            ...
            for choix in article.choix_menu.all():
                role = choix.role.lower()
                nom = None
                if choix.plat_choisi:
                    nom = choix.plat_choisi.nom
                ...
                if role == "viande" and nom:
                    viandes_txt.append(nom)
                elif role == "accompagnement" and nom:
                    accompagnements_txt.append(nom)
                elif nom:
                    role_label = choix.get_role_display() if hasattr(choix, 'get_role_display') else choix.role.capitalize()
                    autres.append(f"{role_label} : {nom}")

            if viandes_txt:
                for viande in viandes_txt:
                    options_list.append(viande)
            if accompagnements_txt:
                for accompagnement in accompagnements_txt:
                    options_list.append(accompagnement)
            for autre in autres:
                options_list.append(autre)

        # 🍔 PLAT CLASSIQUE AVEC OPTIONS
        elif article.plat:
            plat_nom = article.plat.nom
            ...
            if article.options.exists():
                for option in article.options.all():
                    ...
                    if option_nom.startswith('Supplément '):
                        option_nom = '+' + option_nom[11:]
                    elif not option_nom.startswith('+'):
                        option_nom = '+' + option_nom
                    
                    if option.prix_unitaire_ttc and option.prix_unitaire_ttc > 0:
                        option_nom += f" (+{option.prix_unitaire_ttc} €)"
                    
                    options_list.append(option_nom)
                
                print(f"🔧 Options M2M trouvées pour {plat_nom}: {options_list}")
                logger.debug(f"[get_cart_items] Options M2M pour plat #{article.plat.id}: {options_list}")
        ...
        if article.accompagnement:
            accompagnement_info = {
                ...
                'prix': str(article.accompagnement.prix_supplement) if article.accompagnement.prix_supplement else '0.00'
            }
            print(f"🍟 Accompagnement trouvé: {accompagnement_info['nom']} (+{accompagnement_info['prix']}€)")

        # 📋 CONSTRUCTION DE L'ARTICLE FINAL
        cart_item = {
        ...
        print(f"   - Accompagnement: {accompagnement_info['nom'] if accompagnement_info else 'Aucun'}")
    ...
    for item in cart_items:
        print(f"   - {item['plat_nom']} | Accompagnement: {item['accompagnement']['nom'] if item['accompagnement'] else 'Aucun'}")
        if item['options']:
            print(f"     Options:")
...
def preparer_items_stripe(panier):
    ...
    for article in panier.articlepanier_set.all():
        ...
        if not prix_unitaire or prix_unitaire <= 0:
            print(f"[STRIPE] Article ignoré car prix nul : {nom_produit}")
            continue
    ...
    print("[STRIPE] line_items envoyés :", line_items)
    return line_items
...
def creer_session_stripe(line_items, session_key, commande_id, coupon_id=None):
    ...
    print(f"🎯 [DEBUG] BASE_URL utilisé: {settings.BASE_URL}")  # ← MODIFIEZ ICI
    ...
    try:
        session_stripe = stripe.checkout.Session.create(
            ...
            mode='payment',
            success_url=f'{settings.BASE_URL}/success?commande_id={commande_id}',  # ← MODIFIEZ ICI
            cancel_url=f'{settings.BASE_URL}/cancel',  # ← MODIFIEZ ICI
            client_reference_id=str(commande_id),
            discounts=[{'coupon': coupon_id}] if coupon_id else None,
        )
        print(f"✅ [DEBUG] Session Stripe créée: {session_stripe.id}")
...
def valider_commande(request):
    ...
    if any(p['id'] == 'stripe' for p in paiements_possibles):
        try:
            line_items = preparer_items_stripe(panier)
            coupon_id = None
...
def api_commandes_a_valider(request):
    ...
    for commande in commandes_qs:
        ...
        print(f"[DEBUG] Commande {commande.id} : email={commande.email_saisi}, tel={commande.telephone_saisi}, nom={commande.nom_saisi}, prenom={commande.prenom_saisi}")
        logger.info(f"[DEBUG] Commande {commande.id} - Recherche client existant...")
        ...
        print(f"[DEBUG] Commande {commande.id} : {match_count} champ(s) matché(s) → {match_fields}")
        ...
        commandes_list.append({
            'id': commande.id,
            'client__id': commande.client.id if commande.client else None,   # 👈 AJOUTE CETTE LIGNE
            'client__nom': commande.client.nom if commande.client else '',
            'client__prenom': commande.client.prenom if commande.client else '',
            'client__numero_telephone': commande.client.numero_telephone if commande.client else '',
            'montant_total': montant,
...
def extraire_articles_et_aggregats(commande, aggregated_articles=None, filtrer_cuisine=True):
    ...
    # ✅ Ignorer les commandes sans panier
    if not commande.panier:
        print(f"⚠️ COMMANDE {commande.id} ignorée : aucun panier associé.")
    ...
    for article in article_qs:
        ...
        # ----------- SALADE PERSONNALISÉE -----------
        if article.salade_personnalisee:
            salade = article.salade_personnalisee
            nom_article = "Salade personnalisée"
            categorie = "Salades"
            base = f"Base : {salade.base.nom}" if salade.base else None
            sauce = f"Sauce : {salade.sauce.nom}" if salade.sauce else None
            proteines = [p.nom for p in salade.proteines.all()]
            garnitures = [g.nom for g in salade.garnitures.all()]
            toppings = [t.nom for t in salade.toppings.all()]
            supplements = [s.nom for s in salade.supplement.all()]
            raw_items = [base, *proteines, *garnitures, *toppings, *supplements, sauce]
            details_data = [str(item).strip() for item in raw_items if item and str(item).strip()]
            options_str = ", ".join(details_data)
        ...
        # ----------- COUSCOUS PERSONNALISÉ -----------
            ...
            for choix in couscous.choixviandecouscous_set.select_related('viande'):
                ...
                if not choix.est_incluse and choix.viande.supplement_extra:
                    nom += f" (+{choix.viande.supplement_extra}€)"
                viandes.append(nom)
            xl = f"Option XL : {'Oui' if couscous.option_xl else 'Non'}"
            accompagnements = ", ".join(a.nom for a in couscous.accompagnements.all())
            details_data = [*viandes, xl, accompagnements]
            options_str = ", ".join(details_data)
        ...
        # ----------- MENU PERSONNALISÉ -----------
            ...
            choix = []
            viandes_menu = []
            accompagnements_menu = []
            autres_choix = []

            # ⚠️ On filtre les rôles "boisson" et "dessert" selon le contexte
            for c in article.choix_menu.all():
                ...
                # → Cuisine : on skippe boissons/desserts
                # → Bar : on ne prend QUE boissons/desserts
                is_boisson = (role_lower == "boisson")
                is_dessert = (role_lower == "dessert")
                if filtrer_cuisine and (is_boisson or is_dessert):
                    continue
                if not filtrer_cuisine and not (is_boisson or is_dessert):
                    continue

                if c.role == "viande" and getattr(c, "info_text", None):
                    viandes_menu.append(c.info_text)
                elif c.role == "accompagnement" and getattr(c, "info_text", None):
                    accompagnements_menu.append(c.info_text)
                elif c.plat_choisi:
                    autres_choix.append(f"{label} : {c.plat_choisi.nom}")
                elif getattr(c, "info_text", None):
                    autres_choix.append(f"{label} : {c.info_text}")
                elif c.salade:
                    autres_choix.append(f"{label} : Salade personnalisée")
                elif c.couscous:
                    autres_choix.append(f"{label} : Couscous personnalisé")

            if viandes_menu:
                choix.append("viande : " + ", ".join(viandes_menu))
            if accompagnements_menu:
                choix.append("accompagnement : " + ", ".join(accompagnements_menu))
            choix.extend(autres_choix)
            options_str = ", ".join(choix)
            details_data = choix
        ...
        # ----------- PLAT CLASSIQUE -----------
            ...
            # même logique pour filtrer_cuisine/bar
            ...
            if not filtrer_cuisine and not (is_boisson or is_dessert):
                continue
            details_data = [opt.nom_option for opt in article.options.all()]
            options_str = ", ".join(details_data)
        ...
        articles.append({
            ...
            'cuissons': cuisson_data
        })

        # ✅ Agrégation par catégorie + nom
        if categorie not in aggregated_articles:
            aggregated_articles[categorie] = {}

        article_key = f"{nom_article} - {options_str}"
    ...
    try:
        ...
        for menu in menus:
            data.append({
                ...
                "description_longue": "Choisissez un plat, une boisson et un dessert.",
                "prix_unitaire_ttc": str(menu.prix_ttc),  # ✅ OK
                "photo_url": menu.photo.url if menu.photo else "/static/images/default.jpg"
            })

        return JsonResponse({"menus": data})  # ✅ CORRECT ICI
...
def ajouter_menu_personnalise(request):
    ...
    try:
        ...
        choix = data.get("choix", {})      # Ex: {"plat": ["48"], "boisson": ["51"], ...}
        viandes = data.get("viandes", {})  # Ex: {"viande": [ID, ...], "accompagnements": [ID, ...]}
        print("CHOIX :", choix)
        print("VIANDES (accompagnements inclus) :", viandes)
        ...
        # Fonction utilitaire pour normaliser les choix
        ...
        # Fonction de comparaison profonde entre un article existant et la nouvelle config (toutes options/viandes/accompagnements)
        def article_correspond(article, choix_utiles, viandes):
            # Collecte des choix existants dans l'article
            choix_existants = {}
            for c in ChoixMenuArticle.objects.filter(article_panier=article):
                if c.plat_choisi:
                    ...
                    # Ajout des accompagnements et viandes sous forme de texte
                    choix_existants.setdefault(c.role, []).append(("info", c.info_text))

            # On compare chaque rôle/valeur du nouveau menu
            ...
            # On compare aussi les accompagnements et viandes
            # => compare info_text pour chaque accompagnement/viande (par nom)
            for key in ["accompagnements", "incluses"]:
                existing = set(
                    c.info_text for c in ChoixMenuArticle.objects.filter(article_panier=article, role=("accompagnement" if key == "accompagnements" else "viande"))
                    if getattr(c, "info_text", None)
                )
                incoming = set()
                if key in viandes:
                    if key == "accompagnements":
                        from .models import AccompagnementCouscous
                        for acc_id in viandes[key]:
                            try:
                                acc = AccompagnementCouscous.objects.get(id=acc_id)
                                incoming.add(acc.nom)
                            except: pass
                    if key == "incluses":
                        from .models import ViandeCouscous
                        for v_id in viandes[key]:
                            try:
                                v = ViandeCouscous.objects.get(id=v_id)
                                incoming.add(v.nom)
                            except: pass
        ...
        # Normalise la structure des choix reçus
        choix_utiles = {}
        ...
        if not found:
            article = ArticlePanier.objects.create(
                ...
                menu=menu,
                quantite=1,
                prix_total=menu.prix_ttc
            )
            print(f"Nouvel ArticlePanier créé : {article}")

            # Choix classiques (plat, salade, couscous, etc)
            for role, vals in choix.items():
                if not isinstance(vals, list):
                    vals = [vals]
                for val in vals:
                    type_obj, obj_id = detect_type_id(val)
                    viande_data = viandes.get(role, [])
                    if type_obj == "plat" and obj_id:
                        ...
                        from .models import CouscousPersonnalise
                        couscous = CouscousPersonnalise.objects.get(id=obj_id)
                        ChoixMenuArticle.objects.create(
                            article_panier=article,
                            role=role,
                            couscous=couscous,
                            viandes_autorisees=viande_data
                        )
            ...
            print("DEBUG ACCOMPAGNEMENTS :", accompagnements_ids)
            ...
            print("DEBUG VIANDES :", viandes_ids)
            if viandes_ids:
                from .models import ViandeCouscous
                for v_id in viandes_ids:
                    try:
                        viande = ViandeCouscous.objects.get(id=v_id)
                        ChoixMenuArticle.objects.create(
                            article_panier=article,
                            role="viande",
                            info_text=viande.nom
                        )
                        print(f"✅ Ajouté viande : {viande.nom}")
                    except Exception as e:
                        logger.warning(f"ViandeCouscous introuvable (id={v_id}): {e}")
                        print(f"❌ ViandeCouscous introuvable (id={v_id}): {e}")
...
from django.http import JsonResponse
from .models import Menu, AccompagnementCouscous
...
def api_choix_menu(request, menu_id):
    ...
    utilise_couscous = False  # Flag pour savoir si on doit afficher les accompagnements
    ...
    # ⚠️ N'inclure les accompagnements que si on est sur un menu couscous
